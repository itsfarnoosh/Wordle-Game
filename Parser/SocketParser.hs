...
module SocketParser (parseHTTPRequest) where

import           Control.Applicative (Alternative (many, (<|>)))
import           Instances           (Parser (..))
import           JSON                (JsonValue (JNull), json)
import           Parser              (isNot, spaces, string)
...
parseHTTPRequest =
    (,,) <$> (getOrPost <* spaces)
         <*> (spaces *> parseURL)
         <* (spaces *> string "HTTP/1.1" <* spaces)
         <*> ((spaces *> json) <|> pure JNull)
